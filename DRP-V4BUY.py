...
##################   Detroid Focused Pull #########################

print('*********************  Detroid Focused Pull ***********************')
...
### Pull in all TypeID's for Detroid to iterate through
column_names = ["Detroid"]
df = pd.read_csv("FocusedTypeIDMetro.csv", names=column_names)
print(df)

typeid = df.Detroid.to_list()    ####  Using the TypeIDs in the Metro file
print(time)
...
while i in range(2,len(typeid)):
    ...
    i+=1
    try:
        Sales = get_sell_orders(10000005,T_ID) # The Forge = 10000016, Everyshore 10000037, Metropolis 10000042, Detroid 100000005
    except:
        print('url data pull error at id:', i)
    #print (Sales)
    print('index:',i)

    filename = "Detroid_Focused_BUY"+todaydate+".csv"
