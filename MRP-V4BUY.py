...
##################   Metropolis Focused Pull #########################

print('*********************  Metropolus Focused Pull ***********************')
...
### Pull in all TypeID's for Metropolis to iterate through
column_names = ["Metropolis"]
df = pd.read_csv("FocusedTypeIDMetro.csv", names=column_names)
print(df)

typeid = df.Metropolis.to_list()    ####  Using the TypeIDs in the Metropolis file
print(time)
...
while i in range(2,len(typeid)):
    ...
    i+=1
    try:
        Sales = get_sell_orders(10000042,T_ID) # The Forge = 10000016, Everyshore 10000037, Metropolis 10000042
    except:
        print('url data pull error at id:', i)
    #print (Sales)
    print('index:',i)

    filename = "Metropolis_Focused_BUY"+todaydate+".csv"
