...
##################   Lonetrek Focused Pull #########################

print('*********************  Lonetrek Focused Pull ***********************')
...
def get_sell_orders(region_id, type_id, page=1):
    try:
        url = "https://esi.evetech.net/latest/markets/{}/orders/?datasource=tranquility&order_type=buy&page={}&type_id={}".format(region_id, page, type_id)
        r = requests.get(url)
        sell_orders = r.json()
    except:
...
### Pull in all TypeID's for Lonetrek to iterate through
column_names = ["Lone"]
df = pd.read_csv("FocusedTypeIDLone.csv", names=column_names)   ################# Production Type_IDs
#df = pd.read_csv("ReducedTypeIDs.csv", names=column_names)       ################# Initial pull using reduced Type_IDs to get focused Type_IDs
print(df)

typeid = df.Lone.to_list()    ####  Using the TypeIDs in the Lonetrek file
print(time)
...
while i in range(2,len(typeid)):
    ...
    i+=1
    try:
        Sales = get_sell_orders(10000016,T_ID) # The Forge = 10000016, Everyshore 10000037, Metropolis 10000042, Kor-Azor 10000065, Lonetrek 10000016
    except:
        print('url data pull error at id:', i)
    #print (Sales)
    print('index:',i)

    filename = "Lonetrek_Focused_BUY"+todaydate+".csv"
