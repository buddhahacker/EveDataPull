...
for x in range(0,len(new_list),2):   #############this increments the index x by 2 and limit reduced by 1 so I don't get out of bounds condition
    print('x value at top of loop',x)
    my_tuple_list = np.array(new_list)                               ######problem is that in increment through the new_list doesn't need to occur if only pulling 1 fixed order_id
