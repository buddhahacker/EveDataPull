...
##################   Sing Laison Focused Pull #########################

print('*********************  Sing Laison Focused Pull ***********************')
...
column_names = ["Sing"]
df = pd.read_csv("FocusedTypeIDSing.csv", names=column_names)   
#df = pd.read_csv("ReducedTypeIDs.csv", names=column_names)     #########  For Initial Run to get ID's for FOCUSEDTYPEID
print(df)

typeid = df.Sing.to_list()    ####  Using the TypeIDs in the Kor-Azor file
print(time)
...
while i in range(2,len(typeid)):
    ...
    i+=1
    try:
        Sales = get_sell_orders(10000032,T_ID) # The Forge = 10000016, Everyshore 10000037, Metropolis 10000042, Kor-Azor 10000065, Lonetrek 10000016, sing 32
    except:
        print('url data pull error at id:', i)
    #print (Sales)
    print('index:',i)

    filename = "Sing_Focused_BUY"+todaydate+".csv"
