...
column_names = ["Detorid"]
df = pd.read_csv("OreIceTypeIDs.csv", names=column_names)
print(df)

typeid = df.Detorid.to_list()    ####  Using the TypeIDs in the Metro file
print(time)
...
while i in range(2,len(typeid)):
    ...
    filename = "Detroid_ORE_"+todaydate+".csv"
